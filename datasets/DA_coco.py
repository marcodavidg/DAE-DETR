...
class CocoDetection(TvCocoDetection):
    def __init__(self, img_folder, ann_file, transforms, return_masks, is_foggy=False, cache_mode=False, local_rank=0,
                 local_size=1):
...
def make_coco_transforms(image_set):
    ...
    if image_set == 'train':
        return T.Compose([
            ...
            T.RandomSelect(
                T.RandomResize(scales, max_size=1792),
                T.Compose([
                    # T.RandomResize([400, 500, 600]),
                    T.RandomResize([256, 512, 768]),
    ...
    if image_set == 'val':
        return T.Compose([
            ...
            T.RandomResize([768], max_size=1792),
            normalize,
        ])
...
def make_da_transforms(image_set):
    ...
    if image_set == 'train':
        return T.Compose([
            ...
            # T.RandomResize([1000],max_size=1000),
            # T.RandomSelect(
            #     T.RandomResize((200,200)),
            #     T.Compose([
            #         T.RandomResize((200,200)),
            #         T.RandomSizeCrop(384, 600),
            #         T.RandomResize((200,200)),
    ...
    if image_set == 'val':
        return T.Compose([
            # T.RandomResize([800], max_size=1333),
            T.RandomResize([512,768], max_size=1792),
            normalize,
        ])
...
def build_city2foggy_cocostyle_source(image_set, args):
    ...
    PATHS = {
        "train": (root / 'cityscapes' / "leftImg8bit/train",
                  root / 'cityscapes' / "annotations" / f'O2instancesonly_filtered_gtFine_train.json'),
        "val": (root / 'foggy_cityscapes' / "leftImg8bit_foggy/val",
                root / 'foggy_cityscapes' / "annotations" / f'instancesonly_filtered_gtFine_val.json'),
    }

    img_transform = globals()[args.transform]("train" if "train" in image_set else "val")

    img_folder, ann_file = PATHS[image_set]
    dataset = CocoDetection(img_folder, ann_file, is_foggy=(image_set is not "train"), transforms=img_transform,
...
def build_city2foggy_cocostyle(image_set, args):
    ...
    mode = 'instances'
    PATHS = {
        "train_src": (root / 'cityscapes' / "leftImg8bit/train",
                      root / 'cityscapes' / "annotations" / f'O2instancesonly_filtered_gtFine_train.json'),
        "train_tgt": (root / 'foggy_cityscapes' / "leftImg8bit_foggy/train",
                      root / 'foggy_cityscapes' / "annotations" / f'instancesonly_filtered_gtFine_train.json'),
        "val": (root / 'foggy_cityscapes' / "leftImg8bit_foggy/val",
                root / 'foggy_cityscapes' / "annotations" / f'instancesonly_filtered_gtFine_val.json'),
    }
    ...
    img_folder, ann_file = PATHS[image_set]
    dataset = CocoDetection(img_folder, ann_file, is_foggy=(image_set is not 'train_src'), transforms=img_transform,
                            return_masks=args.masks,
                            cache_mode=args.cache_mode, local_rank=get_local_rank(), local_size=get_local_size())
